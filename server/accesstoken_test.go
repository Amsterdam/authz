...
var enc *accessTokenEncoder

func init() {
	enc = newAccessTokenEncoder([]byte("test"), 60, "issuer")
}

func decodeJWT(jwt string) (*accessTokenJWTHeader, *accessTokenJWTPayload, error) {
	var (
...
func TestEncode(t *testing.T) {
	...
	header, payload, err := decodeJWT(jwt)
	if err != nil {
		t.Fatal(err)
	}
